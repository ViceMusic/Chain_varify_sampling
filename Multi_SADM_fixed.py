...
CONFIG = {
    ...
    "message": "FixedFinalWeightSADM_xm0001_x20028_x19971",
}
...
def fixed_weight_sadm_loss(
    ...
    用来验证：
    AdaSADM 的收益到底来自最终权重组合，
    还是来自训练过程中的动态权重变化。
    """
    weights = torch.tensor(
        [fixed_weight_dict[name] for name in layer_names],
        dtype=torch.float,
        device=device
    )
    ...
    # 和 AdaSADM 保持一致：softmax 加权后乘层数
    total = total * len(layer_names)

    return total, weights, loss_items


def get_fixed_layer_weights(args, layer_names):
    weights = torch.tensor(
        [args.fixed_sadm_weights[name] for name in layer_names],
        dtype=torch.float,
        device=args.device
    )
...
def main():
    ...
    logger.info("Experiment: Fixed final SADM layer weights")
    ...
    for exp in range(args.num_exp):
        ...
        fixed_weights = get_fixed_layer_weights(args, sadm_layer_names)

        logger.info("SADM layers: %s", sadm_layer_names)
        logger.info("SADM base coef: %s", args.sadm_base_coef)
        logger.info("Fixed SADM layer weights: %s", fixed_weights.detach().cpu().numpy())
        ...
        for it in range(args.Iteration + 1):
            ...
            for c in range(num_classes):
                ...
                loss_sadm_fixed, sadm_weights, sadm_loss_items = fixed_weight_sadm_loss(
                    layers_real=layers_real,
                    layers_syn=layers_syn,
                    layer_names=sadm_layer_names,
                    base_coef_dict=args.sadm_base_coef,
                    fixed_weight_dict=args.fixed_sadm_weights,
                    device=args.device,
                )
            ...
            if (it % (args.PRINT_EVERY * 10) == 0) or (it == args.Iteration):
                with torch.no_grad():
                    w = get_fixed_layer_weights(args, sadm_layer_names).detach().cpu().numpy()
                    weight_msg = " | ".join([
                        f"{name}={w_i:.4f}"
                        for name, w_i in zip(sadm_layer_names, w)
                    ])
                    logger.info("[Fixed Layer Weights] Iter %d | %s", it, weight_msg)
                    print(f"[Fixed Layer Weights] Iter {it} | {weight_msg}")
            ...
            if it == args.Iteration:
                ...
                save_dict = {
                    ...
                    "sadm_layer_weights": get_fixed_layer_weights(
                        args,
                        sadm_layer_names
                    ).detach().cpu(),
                    "sadm_base_coef": args.sadm_base_coef,
                    "fixed_sadm_weights": args.fixed_sadm_weights,
                    "experiment_type": "fixed_final_weight_sadm",
                }
