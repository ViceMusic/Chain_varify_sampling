...
# =========================================================
# 该版本为完整版本的AdaSADM
# 也就是GCCE上的版本，和Github上的交付仓库也是一致的
# 后续作为模板使用
# =========================================================

# =========================================================
# 修改，该完整版本为完整版本的完整版本
# 保留先验的内容的权重，但是把pointnet单独的匹配层也加上了
# 后续作为模板使用
# =========================================================
...
CONFIG = {
    ...
    # SADM 总分支系数。设为 1.0 表示保持原 AdaSADM 主损失尺度不变。
    # 之前代码里 loss_sadm_weighted 没有额外乘 coef，本质上等价于 sadm_loss_coef = 1.0。
    "sadm_loss_coef": 1.0,


    # =====================================================
    # Adaptive head feature matching 设置
    # =====================================================
    "use_learnable_feature_weight": True,

    "feature_layers": ["f1", "f2", "f3"],

    "feature_base_coef": {     # 这个玩意是预设尺度，就是平时n*loss的那个n
        "f1": 0.3,
        "f2": 0.3,
        "f3": 0.3,
    },

    "feature_weight_prior": {  # 先验softmax权重
        "f1": 0.50,
        "f2": 0.30,
        "f3": 0.20,
    },

    "lr_feature_weight": 0.01,

    "feature_weight_kl": 0.001,

    # MSE 总分支系数。用于控制 f1/f2/f3 这组 head feature matching 的整体强度。
    "feature_loss_coef": 1.0,

    # M3D 总分支系数。保持原代码中的 loss_m3d * 0.001 不变，只是显式写到 CONFIG 里。
    "m3d_loss_coef": 0.001,

    "message": "AdaptiveMultiSADM_with_MSE_contribution_log",
}
...
# =========================================================
# Adaptive Multi-layer MSE 工具函数
# =========================================================

def feature_layer_loss(feat_real, feat_syn):
    """
    feat_real / feat_syn: [B, C]
    用于 f1/f2/f3 这类全局向量特征。
    """
    real = feat_real.detach().mean(dim=0)
    syn = feat_syn.mean(dim=0)

    return torch.mean((real - syn) ** 2)
...
def weighted_feature_loss(
    ...
    """
    对 f1/f2/f3 这类 [B, C] 特征计算 MSE matching，
    并使用 learnable softmax 权重融合。
    """
    weights = torch.softmax(weight_logits, dim=0)
    ...
    for i, name in enumerate(layer_names):
        layer_loss = feature_layer_loss(layers_real[name], layers_syn[name])
        base_coef = base_coef_dict[name]
...
def feature_weight_kl_loss(weight_logits, prior_values):
    eps = 1e-12
    weights = torch.softmax(weight_logits, dim=0)
...
def main():
    ...
    for exp in range(args.num_exp):
        ...
        # =========================================================
        # Learnable layer MSE weights
        # =========================================================

        feature_layer_names = list(args.feature_layers)

        feature_prior_values = torch.tensor(
            [args.feature_weight_prior[name] for name in feature_layer_names],
            dtype=torch.float,
            device=args.device
        )
        feature_prior_values = feature_prior_values / feature_prior_values.sum()

        feature_weight_logits = torch.log(
            feature_prior_values + 1e-12
        ).detach().clone().requires_grad_(True)

        logger.info("Feature layers: %s", feature_layer_names)
        logger.info("Feature base coef: %s", args.feature_base_coef)
        logger.info("Initial feature layer prior: %s", feature_prior_values.detach().cpu().numpy())

        # =========================================================
        # 注意：M3D 保持原样，不设置 learnable layer weights。
        # 本版本只给 SADM(x_m/x_2/x_1) 和 MSE(f1/f2/f3) 设置自适应层权重。
        # =========================================================
        ...
        # 追加三层的权重优化器
        optimizer_feature_weight = torch.optim.Adam(
            [feature_weight_logits],
            lr=args.lr_feature_weight
        )
        ...
        for it in range(args.Iteration + 1):
            ...
            feature_raw_loss_tracker = {
                name: []
                for name in feature_layer_names
            }

            feature_contrib_tracker = {
                name: []
                for name in feature_layer_names
            }
            ...
            for c in range(num_classes):
                ...
                # =================================================
                # Adaptive Multi-layer SADM: x_m / x_2 / x_1
                # =================================================
                ...
                # =================================================
                # Adaptive feature matching: f1 / f2 / f3
                # =================================================
                loss_feature_weighted, feature_weights, feature_loss_items, feature_contrib_items = weighted_feature_loss(
                    layers_real=layers_real,
                    layers_syn=layers_syn,
                    layer_names=feature_layer_names,
                    base_coef_dict=args.feature_base_coef,
                    weight_logits=feature_weight_logits,
                )

                # =================================================
                # M3D: x_gf global feature matching
                # =================================================
                # M3D 保持原来的 x_gf 匹配方式，不参与 SADM/MSE 的层级 contribution ratio。
                # 它的整体强度由 args.m3d_loss_coef 控制，默认仍为 0.001。
                ...
                # =================================================
                # 记录 SADM/MSE 的 per-layer raw loss 和 effective contribution
                # =================================================
                # 注意：
                # 1. weighted_sadm_loss / weighted_feature_loss 返回的 contrib_items
                #    已经包含 layer softmax weight、base_coef、raw_loss 和 num_layers。
                # 2. 这里再乘 sadm_loss_coef / feature_loss_coef，
                #    是为了让最终打印的 contribution ratio 和真正进入总 loss 的比例一致。
                # 3. M3D 是独立全局损失，不放进 SADM+MSE 的 per-layer contribution ratio。
                for name in sadm_layer_names:
                    raw_loss_tracker[name].append(sadm_loss_items[name])
                    contrib_tracker[name].append(
                        sadm_contrib_items[name] * args.sadm_loss_coef
                    )

                for name in feature_layer_names:
                    feature_raw_loss_tracker[name].append(feature_loss_items[name])
                    feature_contrib_tracker[name].append(
                        feature_contrib_items[name] * args.feature_loss_coef
                    )

                # =================================================
                # 三种损失分支：
                # 1. SADM: x_m/x_2/x_1 的 point-wise sorted response matching
                # 2. MSE : f1/f2/f3 的 head feature mean matching
                # 3. M3D : x_gf 的原始 M3D matching，保持原样
                # =================================================
                loss1 = torch.tensor(0.0, device=args.device)
                loss1 += args.sadm_loss_coef * loss_sadm_weighted
                loss1 += args.feature_loss_coef * loss_feature_weighted
                loss1 += args.m3d_loss_coef * loss_m3d
            ...
            if args.use_learnable_feature_weight and args.feature_weight_kl > 0:
                loss += args.feature_weight_kl * layer_weight_kl_loss(
                    feature_weight_logits,
                    feature_prior_values
                )

            optimizer_img.zero_grad() # 图像优化
            optimizer_theta.zero_grad() # 角度优化
            optimizer_layer_weight.zero_grad() # x层权重优化
            optimizer_feature_weight.zero_grad() # f层权重优化
            ...
            if (it % (args.PRINT_EVERY * 10) == 0) or (it == args.Iteration):
                with torch.no_grad():
                    # -----------------------------
                    # SADM branch: x_m / x_2 / x_1
                    # -----------------------------
                    sadm_w = torch.softmax(layer_weight_logits, dim=0).detach().cpu().numpy()

                    sadm_raw_mean = {}
                    sadm_contrib_mean = {}

                    for name in sadm_layer_names:
                        sadm_raw_mean[name] = torch.stack(raw_loss_tracker[name]).mean().item()
                        sadm_contrib_mean[name] = torch.stack(contrib_tracker[name]).mean().item()

                    sadm_weight_msg = " | ".join([
                        f"{name}={w_i:.4f}"
                        for name, w_i in zip(sadm_layer_names, sadm_w)
                    ])

                    sadm_raw_msg = " | ".join([
                        f"{name}_raw={sadm_raw_mean[name]:.6f}"
                        for name in sadm_layer_names
                    ])

                    sadm_contrib_msg = " | ".join([
                        f"{name}_contrib={sadm_contrib_mean[name]:.6f}"
                        for name in sadm_layer_names
                    ])
                    ...
                    # MSE branch: f1 / f2 / f3
                    # -----------------------------
                    mse_w = torch.softmax(feature_weight_logits, dim=0).detach().cpu().numpy()

                    mse_raw_mean = {}
                    mse_contrib_mean = {}

                    for name in feature_layer_names:
                        mse_raw_mean[name] = torch.stack(feature_raw_loss_tracker[name]).mean().item()
                        mse_contrib_mean[name] = torch.stack(feature_contrib_tracker[name]).mean().item()

                    mse_weight_msg = " | ".join([
                        f"{name}={w_i:.4f}"
                        for name, w_i in zip(feature_layer_names, mse_w)
                    ])

                    mse_raw_msg = " | ".join([
                        f"{name}_raw={mse_raw_mean[name]:.6f}"
                        for name in feature_layer_names
                    ])

                    mse_contrib_msg = " | ".join([
                        f"{name}_contrib={mse_contrib_mean[name]:.6f}"
                        for name in feature_layer_names
                    ])
                    ...
                    # -----------------------------
                    # 这里把 SADM 和 MSE 的所有 layer contribution 放在同一个分母下计算比例。
                    # 这样可以直接看 x_m/x_2/x_1/f1/f2/f3 在主匹配损失中的相对贡献。
                    # M3D 是单独的 x_gf 全局损失，不参与这个 layer-wise ratio。
                    combined_contrib = {}
                    combined_contrib.update({f"SADM:{k}": v for k, v in sadm_contrib_mean.items()})
                    combined_contrib.update({f"MSE:{k}": v for k, v in mse_contrib_mean.items()})

                    combined_sum = sum(combined_contrib.values()) + 1e-12
                    combined_ratio_msg = " | ".join([
                        f"{name}_ratio={value / combined_sum:.4f}"
                        for name, value in combined_contrib.items()
                    ])

                    sadm_total = sum(sadm_contrib_mean.values())
                    mse_total = sum(mse_contrib_mean.values())
                    branch_sum = sadm_total + mse_total + 1e-12
                    branch_ratio_msg = (
                        f"SADM_total_ratio={sadm_total / branch_sum:.4f} | "
                        f"MSE_total_ratio={mse_total / branch_sum:.4f}"
                    )

                    logger.info("[SADM Weights] Iter %d | %s", it, sadm_weight_msg)
                    logger.info("[SADM Raw Loss] Iter %d | %s", it, sadm_raw_msg)
                    logger.info("[SADM Contribution] Iter %d | %s", it, sadm_contrib_msg)

                    logger.info("[MSE Weights] Iter %d | %s", it, mse_weight_msg)
                    logger.info("[MSE Raw Loss] Iter %d | %s", it, mse_raw_msg)
                    logger.info("[MSE Contribution] Iter %d | %s", it, mse_contrib_msg)

                    logger.info("[Combined Contribution Ratio: SADM+MSE] Iter %d | %s", it, combined_ratio_msg)
                    logger.info("[Branch Contribution Ratio: SADM vs MSE] Iter %d | %s", it, branch_ratio_msg)

                    print(f"[SADM Weights] Iter {it} | {sadm_weight_msg}")
                    print(f"[SADM Raw Loss] Iter {it} | {sadm_raw_msg}")
                    print(f"[SADM Contribution] Iter {it} | {sadm_contrib_msg}")

                    print(f"[MSE Weights] Iter {it} | {mse_weight_msg}")
                    print(f"[MSE Raw Loss] Iter {it} | {mse_raw_msg}")
                    print(f"[MSE Contribution] Iter {it} | {mse_contrib_msg}")

                    print(f"[Combined Contribution Ratio: SADM+MSE] Iter {it} | {combined_ratio_msg}")
                    print(f"[Branch Contribution Ratio: SADM vs MSE] Iter {it} | {branch_ratio_msg}")
            ...
            if it == args.Iteration:
                ...
                save_dict = {
                    ...
                    "sadm_loss_coef": args.sadm_loss_coef,

                    "feature_layer_names": feature_layer_names,
                    "feature_layer_weights": torch.softmax(
                        feature_weight_logits,
                        dim=0
                    ).detach().cpu(),
                    "feature_weight_logits": feature_weight_logits.detach().cpu(),
                    "feature_weight_prior": feature_prior_values.detach().cpu(),
                    "feature_base_coef": args.feature_base_coef,
                    "feature_loss_coef": args.feature_loss_coef,

                    "m3d_loss_coef": args.m3d_loss_coef,
                    "experiment_type": "adasadm_with_mse_contribution_log",
                }
