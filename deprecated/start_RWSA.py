    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration + 1):
            ...
            for c in range(num_classes):
                ...
                # =====================================================
                # RWSA: Rank-Weighted Semantic Alignment
                # =====================================================
                # layers_real["x_m"] / layers_syn["x_m"] 的形状是 [B, C, N]
                # B: batch 内点云数量
                # C: feature channel 数
                ...
                #
                # 先沿 dim=2 排序：
                # 每个 channel 内部的 N 个点按响应值从大到小排列。
                # 排序后比较的不是点对点位置，而是每个 channel 的响应分布曲线。
                sorted_real = torch.sort(
                    layers_real["x_m"], dim=2, descending=True
                )[0].detach()

                sorted_syn = torch.sort(
                    layers_syn["x_m"], dim=2, descending=True
                )[0]

                # -----------------------------------------------------
                # Rank-aware weighting
                # -----------------------------------------------------
                # 原始 SADM 是对排序后的所有位置直接 mean，
                # 等价于认为 rank=1 和 rank=N 的响应同样重要。
                #
                # RWSA 改成按 rank 加权：
                # 排名越靠前，说明响应越高，权重越大；
                # 排名越靠后，说明响应越低，权重越小。
                ...
                rank = torch.arange(
                    R,
                    device=sorted_syn.device,
                    dtype=sorted_syn.dtype
                )

                # tau 控制权重衰减速度：
                # tau 大：接近普通平均，类似原始 SADM
                # tau 小：更关注 top-ranked responses，接近 Top-k
                tau = args.tau * R

                weight = torch.softmax(-rank / tau, dim=0).view(1, 1, R)

                # 对排序后的 rank 维度做加权求和，得到 [B, C]
                real_weighted = (sorted_real * weight).sum(dim=2)
                syn_weighted = (sorted_syn * weight).sum(dim=2)

                # 再对 batch 做平均，得到 real/syn 的 channel-wise 表示 [C]
                real = real_weighted.mean(dim=0)
                syn = syn_weighted.mean(dim=0)

                # RWSA feature distribution matching loss
                loss1 = (((real - syn) ** 2).sum()).mean() * 0.2
