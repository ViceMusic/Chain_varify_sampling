...
# 角度转化为别的函数，主要是为了生成旋转矩阵，后面会用到这个旋转矩阵来对 synthetic 数据进行旋转变换。
...
def main():
    ...
    for exp in range(args.num_exp):
        ...
        # 每个样本合成样本都有一个旋转角度，这里也是分channel统一管理了，实际上和 synthetic 数据是对应的，每个 synthetic 数据都有一个对应的旋转角度，这些旋转角度是需要学习的参数，所以设置了 requires_grad=True。
        ...
        # 优化器2：优化旋转角度，这里对不同轴的旋转角度设置了不同的学习率，可能是因为在某些轴上的旋转对性能的影响更大，所以设置了更高的学习率。
