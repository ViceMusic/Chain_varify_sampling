...
# =========================================================
# Script note
# 基于 AdaptiveMultiSADM_with_contribution_log 版本修改：
# 在 weighted_sadm_loss() 中按逐点层通道数加入自适应 channel_norm，
# 即以 layer_names[-1] 的通道数作为基准，对每层 raw SADM loss 乘 C_ref / C_i。
# 该版本用于检查 layer contribution 的阶段变化是否受通道数差异影响。

# 在损失按照通道数据取得平均以后，这里给模型的权重值都初始化成固定的
# =========================================================
...
CONFIG = {
    ...
    # base_coef 是相对尺度校准项
    # softmax weight 是自适应门控权重，这里是人为调参，固定住
    "sadm_base_coef": {
        "x_m": 0.2,
        "x_2": 0.2,
        "x_1": 0.2,
...
def weighted_sadm_loss(
    ...
    ref_channels = layers_real[layer_names[-1]].shape[1]  # channel-count normalization baseline, e.g., x_1=64
    for i, name in enumerate(layer_names):
        layer_loss = sadm_layer_loss(layers_real[name], layers_syn[name])
        channel_norm = ref_channels / layers_real[name].shape[1]
        base_coef = base_coef_dict[name]

        # 未乘 num_layers 前的贡献
        layer_contrib = weights[i] * base_coef * channel_norm * layer_loss
