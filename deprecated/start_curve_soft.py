    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration + 1):
            ...
            for c in range(num_classes):
                ...
                # 对 batch 平均：[B, C, N] -> [C, N]
                real = sorted_real.mean(dim=0)
                syn  = sorted_syn.mean(dim=0)

                # =====================================================
                # 1. value loss：仍然保持原始 SADM 写法
                # =====================================================
                loss_value = (((real - syn) ** 2).sum(dim=0)).mean()

                # =====================================================
                # 2. soft rank weight：rank 越靠前，权重越大
                # =====================================================
                R = real.size(1)
                rank = torch.arange(R, device=real.device, dtype=real.dtype)

                tau = 0.25 * R
                w = torch.softmax(-rank / tau, dim=0)  # [R]

                # =====================================================
                # 3. weighted slope loss：加权比较下降趋势
                # =====================================================
                real_slope = real[:, 1:] - real[:, :-1]
                syn_slope  = syn[:, 1:] - syn[:, :-1]

                diff_slope = (real_slope - syn_slope) ** 2  # [C, R-1]

                w_slope = w[:-1]
                w_slope = w_slope / (w_slope.sum() + 1e-6)

                loss_slope = (diff_slope * w_slope.view(1, -1)).sum(dim=1).mean()

                # =====================================================
                # 4. weighted curvature loss：加权比较弯曲程度
                # =====================================================
                real_curve = real[:, 2:] - 2 * real[:, 1:-1] + real[:, :-2]
                syn_curve  = syn[:, 2:] - 2 * syn[:, 1:-1] + syn[:, :-2]

                diff_curve = (real_curve - syn_curve) ** 2  # [C, R-2]

                w_curve = w[1:-1]
                w_curve = w_curve / (w_curve.sum() + 1e-6)

                loss_curve = (diff_curve * w_curve.view(1, -1)).sum(dim=1).mean()
