...
def select_kcenter_in_class(descs, ppc):
    num_samples = descs.shape[0]
    if num_samples == 0:
        raise ValueError("cannot run k-center on an empty class")

    class_mean = descs.mean(dim=0, keepdim=True)
    first = int(torch.cdist(descs, class_mean).view(-1).argmin().item())
...
def select_kcenter_indices(dst_train, indices_class, ppc, coord_dim, npoints):
    ...
    for c, indices in enumerate(indices_class):
        ...
        selected_local = select_kcenter_in_class(descs, ppc)
...
def main():
    ...
    for exp in range(args.num_exp):
        logger.info("\n================== Exp %d ==================\n", exp)
        selected_indices_by_class = select_kcenter_indices(dst_train, indices_class, args.ppc, coord_dim, npoints)
