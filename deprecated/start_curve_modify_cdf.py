...
CONFIG = {
    ...
    "message":"curves_modify_cdf" #这个变量用来给生成结果做标识的 
}
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration + 1):
            ...
            for c in range(num_classes):
                ...
                # =====================================================
                # 1. Channel-wise sorting
                # =====================================================
                # layers_real["x_m"] / layers_syn["x_m"]: [B, C, N]
                # B: batch 内点云数量
                # C: feature channel 数
                # N: 每个点云的点数
                #
                # 对 dim=2 排序，即每个 channel 内部按响应值从大到小排列。
                # 排序后丢失空间对应关系，只保留每个 channel 的响应分布曲线。
                ...
                # 2. 只取 top-rank 高响应区域
                # =====================================================
                # top_ratio 越小，越关注最前面的高响应区域；
                # 建议先用 0.2，如果想更激进可以试 0.1。
                R = real.size(1)
                top_ratio = 0.2
                k = max(3, int(top_ratio * R))
                ...
                # 3. value loss：比较曲线高度
                # =====================================================
                loss_value = ((real_top - syn_top) ** 2).mean()

                # =====================================================
                # 4. slope loss：比较曲线下降趋势
                # =====================================================
                real_slope = real_top[:, 1:] - real_top[:, :-1]
                syn_slope = syn_top[:, 1:] - syn_top[:, :-1]

                loss_slope = ((real_slope - syn_slope) ** 2).mean()

                # =====================================================
                # 5. curvature loss：比较曲线弯曲变化
                # =====================================================
                real_curve = real_top[:, 2:] - 2 * real_top[:, 1:-1] + real_top[:, :-2]
                syn_curve = syn_top[:, 2:] - 2 * syn_top[:, 1:-1] + syn_top[:, :-2]

                loss_curve = ((real_curve - syn_curve) ** 2).mean()

                # =====================================================
                # 6. CDF loss：比较前 k 个高响应的累计趋势
                # =====================================================
                # cumsum 后，rank i 表示前 i 个高响应点的累计贡献。
                # 它比单点 value 更关注整体趋势。
                real_cum = torch.cumsum(real_top, dim=1)
                syn_cum = torch.cumsum(syn_top, dim=1)

                loss_cdf = ((real_cum - syn_cum) ** 2).mean()
