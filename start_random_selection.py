...
def select_random_indices(indices_class, ppc):
    selected_indices_by_class = {}
    for c, indices in enumerate(indices_class):
        if len(indices) == 0:
            raise ValueError(f"class {c} has no training samples")
        replace = len(indices) < ppc
        selected = np.random.choice(indices, size=ppc, replace=replace).tolist()
...
def main():
    ...
    for exp in range(args.num_exp):
        logger.info("\n================== Exp %d ==================\n", exp)
        selected_indices_by_class = select_random_indices(indices_class, args.ppc)
